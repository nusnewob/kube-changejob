...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name kube-changejob-builder
	$(CONTAINER_TOOL) buildx use kube-changejob-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm kube-changejob-builder
	rm Dockerfile.cross
...
GO_MODERNIZE_VERSION ?= latest

>>>>>>> tmp-original-16-06-26-01-06
.PHONY: kustomize
...
HELM_NAMESPACE ?= kube-changejob-system
## Name of the Helm release
HELM_RELEASE ?= kube-changejob
